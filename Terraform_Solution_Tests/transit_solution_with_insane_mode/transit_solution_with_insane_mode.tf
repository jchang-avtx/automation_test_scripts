## Creates Transit VPC and Aviatrix Transit GW with insane mode enabled
## Connects Aviatrix Transit GW to on-prem through AWS VGW
## Creates a couple of Spoke VPCs and Aviatrix GWs with insane mode enabled
...
resource "aviatrix_transit_gateway" "AVX-Transit-GW" {
  ...
  insane_mode              = true
  gw_size                  = "c5.large"
  insane_mode_az           = join("",[var.aws_region,"a"])
...
resource "null_resource" "ping" {
  ...
  provisioner "local-exec" {
    command = "scp -i ${var.private_key} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null transit_solution_with_insane_mode.py ${var.ssh_user}@${module.aws-vpc.ubuntu_public_ip[0]}:/tmp/"
  }

  provisioner "remote-exec" {
    inline = [
      "python3 /tmp/transit_solution_with_insane_mode.py --ping_list ${join(",",[for i in [1,2,4,5]: module.aws-vpc.ubuntu_private_ip[i]])}"
    ]
    connection {
      type = "ssh"
      user = var.ssh_user
