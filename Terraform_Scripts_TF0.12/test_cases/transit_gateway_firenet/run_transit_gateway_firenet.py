"""
run_transit_gateway_firenet.py

Test case for AWS Transit Gateway (FireNet) Terraform resource/ use-case
...
log.info("      2. Create AWS transit gateway (FireNet)")
...
try:
    ...
    tf.import_test("transit_gateway", "firenet_transit_gw")
except tf.subprocess.CalledProcessError as err:
    log.exception(err.stderr.decode())
    log.info("-------------------- RESULT --------------------")
...
try:
    ...
    log.debug("     disableFirenet: Disabling FireNet-availability feature...")
    tf.update_test("disableFirenet")
except tf.subprocess.CalledProcessError as err:
    log.exception(err.stderr.decode())
    log.info("-------------------- RESULT --------------------")
