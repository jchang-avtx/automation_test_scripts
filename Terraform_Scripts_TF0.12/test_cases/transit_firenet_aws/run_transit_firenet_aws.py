"""
run_transit_firenet_aws.py

Test case for building out AWS Transit FireNet solution - tests FireNet policies and Management Access
...
log.info("      2. Create AWS Transit FireNet solution")
log.info("      3. Perform terraform import to identify deltas for FireNet policies, Firewall Management Access")
...
try:
    ...
    log.debug("     Verifying import for transit_firenet_policy (1)...")
    tf.import_test("transit_firenet_policy", "transit_firenet_policy_1")
    log.debug("     Verifying import for transit_firenet_policy (2)...")
    tf.import_test("transit_firenet_policy", "transit_firenet_policy_2")
    log.debug("     Verifying import for transit_firenet_firewall_management...")
    tf.import_test("firewall_management_access", "transit_firenet_firewall_management_1")
except tf.subprocess.CalledProcessError as err:
    log.exception(err.stderr.decode())
    log.info("-------------------- RESULT --------------------")
