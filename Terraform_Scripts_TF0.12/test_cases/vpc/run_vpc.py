...
Test case for VPC Terraform resource/ use-case
...
log.info("      2. Create AWS, ARM, GCP, AWS GovCloud VPCs")
...
try:
    ...
    log.debug("     Importing regular AWS VPC...")
    tf.import_test("vpc", "test_aws_vpc")
    log.debug("     Importing transit AWS VPC...")
    tf.import_test("vpc", "test_aws_transit_vpc")
    log.debug("     Importing FireNet AWS VPC...")
    tf.import_test("vpc", "test_aws_firenet_vpc")
    log.debug("     Importing GCP VPC...")
    tf.import_test("vpc", "test_gcp_vpc")
    log.debug("     Importing regular ARM VNet...")
    tf.import_test("vpc", "test_arm_vnet")
    log.debug("     Importing FireNet ARM VNet...")
    tf.import_test("vpc", "test_arm_firenet_vnet")
    log.debug("     Importing regular AWS Gov VPC...")
    tf.import_test("vpc", "test_aws_gov_vpc")
    log.debug("     Importing transit AWS Gov VPC...")
    tf.import_test("vpc", "test_aws_gov_transit_vpc")
    log.debug("     Importing FireNet AWS Gov VPC...")
    tf.import_test("vpc", "test_aws_gov_firenet_vpc")
except tf.subprocess.CalledProcessError as err:
    log.exception(err.stderr.decode())
    log.info("-------------------- RESULT --------------------")
