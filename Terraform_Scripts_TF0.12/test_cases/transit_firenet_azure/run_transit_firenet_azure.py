"""
run_transit_firenet_azure.py

Test case for building out Azure Transit FireNet solution - tests Azure Native Spoke Peering
...
log.info("      2. Create Azure Transit FireNet solution")
log.info("      3. Perform terraform import to identify deltas for Azure Native Spoke Peering")
...
try:
    ...
    log.debug("     Verifying import for azure_spoke_native_peering...")
    tf.import_test("azure_spoke_native_peering", "arm_transit_firenet_spoke_native_peer")
except tf.subprocess.CalledProcessError as err:
    log.exception(err.stderr.decode())
    log.info("-------------------- RESULT --------------------")
